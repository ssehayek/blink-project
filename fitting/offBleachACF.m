% ACF of blinking and off-state bleaching (with time-integration). See demo
% for usage when fitting.
%
% INPUT VARIABLES
%
...
%
function [out] = offBleachACF(params,tau_vec,n_frames,...
    varargin)
...
p0 = s.a/K;
Kp = K + s.kp;
d = sqrt(Kp^2-4*s.b*s.kp);

L1 = (Kp+d)/2;
L2 = (Kp-d)/2;

v_11 = (Kp+d-2*s.kp)/(2*s.kp);
v_12 = -L1/s.kp;
v_21 = (Kp-d-2*s.kp)/(2*s.kp);
v_22 = -L2/s.kp;
